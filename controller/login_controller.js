    ...
    router.route('/')
        .get(function (req, res) {
            conn.connect().then(function () {
                var sqlQuery = "select kode_menu, nama_menu, link from MS_WEBMENU";
        ...
        .post(function (req, res) {
            conn.connect().then(function () {
                ...
                transaction.begin().then(function () {
                    ...
                    request.input("us", sql.VarChar(50), req.body.username)
                    request.input("pass", sql.Decimal(18, 0), req.body.password)
                    request.execute("qsp_login").then(function () {
                    ...
                    conn.close();
                    res.status(400).send("Error-3 while load data");
                });
            }).catch(function (err) {
                conn.close();
                res.status(400).send("Error-4 while load data");
            });
        });
